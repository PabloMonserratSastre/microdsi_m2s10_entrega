...
// =====================
// Lab · Plantillas por track (para arrancar rápido)
// =====================
const LAB_TEMPLATES = {
  itsm: {
    inventory: [
      "Registrar ticket ITSM",
      "Clasificar tipo de ticket (incidente/solicitud)",
      "Priorizar severidad y urgencia",
      "Detectar duplicados y relacionar incidencias",
      "Asignar grupo resolutor",
      "Solicitar información adicional al usuario",
      "Ejecutar troubleshooting L1",
      "Escalar a L2/L3",
      "Programar cambio estándar",
      "Comunicar estado y ETA",
      "Cerrar ticket y capturar causa raíz",
      "Actualizar base de conocimiento (KB)"
    ].join("\n"),
    prioritization: [
      "Clasificar y enrutar ticket | 5 | 3 | 3 | -1 | Supuesto: alto volumen diario; misrouting provoca retrabajo y SLA",
      "Solicitar información adicional | 4 | 2 | 2 | 0 | Condición: muchas incidencias llegan incompletas; ida/vuelta frecuente",
      "Detectar duplicados | 4 | 3 | 2 | -1 | Condición: picos generan tickets repetidos; mejora visibilidad de incidentes",
      "Priorizar severidad y urgencia | 5 | 4 | 4 | -3 | Riesgo: error crítico en P1; requiere reglas y auditoría",
      "Actualizar KB | 3 | 3 | 1 | -1 | Condición: sin KB sube AHT; valor medio pero continuo"
    ].join("\n"),
    sipoc: [
      "Suppliers: Usuario final; Portal ITSM; Monitorización; Catálogo de servicios; Equipos L2/L3",
      "Inputs: Texto del ticket (PII: posible); Categoría (PII: no); Impacto/Urgencia (PII: no); Historial/adjuntos (PII: posible); Matriz de enrutado (PII: no)",
      "Process: 1) Validar y normalizar entrada 2) Clasificar tipo/categoría 3) Estimar severidad (P1–P4) 4) Enrutar a grupo resolutor 5) Solicitar datos si faltan 6) Registrar decisión para auditoría",
      "Outputs: Ticket en cola correcta; Severidad asignada; Solicitud de datos enviada / escalado",
      "Customers: Usuario final; Service Desk; Equipos resolutores"
    ].join("\n"),
    decision_note: [
      "Decisión final:",
      "- Proceso ganador: Clasificar y enrutar ticket",
      "- 2 criterios (dato/condición):",
      "  1) Condición: alto volumen diario → cada error de enrutado multiplica retrabajo (triage + reasignación).",
      "  2) Condición: SLA sensible en P1 → priorizar reducción de misrouting crítico sobre micro-optimizar esfuerzo.",
      "- Restricción dominante: Auditoría + control de criticidad (evitar P1 mal clasificados).",
      "- Trade-off: Acepto más esfuerzo (reglas + logs + revisión) para bajar el riesgo de misrouting."
    ].join("\n")
  },
  ...
  hr: {
    inventory: [
      ...
      "Responder dudas de nómina",
      "Gestionar cambio de datos personales",
      "Emitir certificado laboral",
      "Crear alta de empleado",
      "Gestionar onboarding",
      "Gestionar offboarding",
      "Actualizar contrato/anexo",
      "Gestionar permisos retribuidos",
      "Gestionar incidencias de fichaje",
      "Gestionar consultas de beneficios"
    ].join("\n"),
    prioritization: [
      "Gestionar solicitud de vacaciones | 4 | 2 | 2 | 0 | Condición: volumen estacional y repetitivo; poco PII",
      "Gestionar incidencias de fichaje | 4 | 3 | 3 | -2 | Condición: reglas claras; impacto en payroll",
      "Responder dudas de nómina | 5 | 4 | 5 | -4 | Riesgo PII alto; requiere RBAC + minimización de logs",
      "Emitir certificado laboral | 3 | 3 | 4 | -4 | Riesgo compliance; documentos formales",
      "Gestionar cambio de datos personales | 4 | 3 | 5 | -4 | Riesgo PII; trazabilidad obligatoria"
    ].join("\n"),
    sipoc: [
      "Suppliers: Empleado; Manager; HRIS; Payroll; Legal",
      "Inputs: Identidad del empleado (PII: sí); Tipo solicitud (PII: no); Documentos adjuntos (PII: sí); Políticas internas (PII: no)",
      "Process: 1) Autenticar usuario 2) Capturar intención y validar mínimos 3) Consultar política/estado en HRIS 4) Resolver o derivar a HR 5) Registrar trazabilidad mínima 6) Confirmar al empleado",
      "Outputs: Respuesta al empleado; Ticket HR creado; Acción registrada",
      "Customers: Empleado; HR Ops; Payroll"
    ].join("\n"),
    decision_note: [
      "Decisión final:",
      "- Proceso ganador: Gestionar solicitud de vacaciones",
      "- 2 criterios (dato/condición):",
      "  1) Condición: alto volumen estacional → mejora rápida del tiempo de respuesta.",
      "  2) Condición: riesgo PII menor vs nómina → automatizar primero con menor exposición.",
      "- Restricción dominante: No registrar PII en logs.",
      "- Trade-off: Priorizo quick win de bajo riesgo antes de atacar nómina (alto riesgo)."
    ].join("\n")
  },

  proc: {
    inventory: [
      "Crear solicitud de compra (PR)",
      "Validar presupuesto",
      ...
      "Recepcionar mercancía/servicio",
      "Conciliar factura (3-way match)",
      "Gestionar incidencias con proveedor",
      "Actualizar maestro de proveedores",
      "Renegociar contrato"
    ].join("\n"),
    prioritization: [
      "Conciliar factura (3-way match) | 5 | 4 | 3 | -2 | Condición: excepciones frecuentes; impacto en pagos",
      "Solicitar cotizaciones (RFQ) | 4 | 3 | 2 | -1 | Condición: tareas repetitivas; mejora velocidad",
      "Emitir orden de compra (PO) | 4 | 3 | 3 | -2 | Condición: requiere validaciones y aprobaciones",
      "Gestionar incidencias con proveedor | 3 | 2 | 2 | -1 | Condición: alto intercambio de mensajes",
      "Actualizar maestro de proveedores | 4 | 4 | 4 | -4 | Riesgo compliance/PII; esfuerzo alto"
    ].join("\n"),
    sipoc: [
      "Suppliers: Solicitante; Catálogo; ERP; Proveedor; Finanzas",
      "Inputs: PR (PII: posible); Presupuesto (PII: no); Cotizaciones (PII: posible); PO (PII: posible); Recepción (PII: no)",
      "Process: 1) Capturar PR 2) Validar presupuesto 3) Generar/validar PO 4) Confirmar recepción 5) Conciliar factura 6) Registrar excepción si falla",
      "Outputs: Factura conciliada; Excepción registrada; Pago autorizado",
      "Customers: Finanzas; Solicitante; Proveedor"
    ].join("\n"),
    decision_note: [
      "Decisión final:",
      "- Proceso ganador: Conciliar factura (3-way match)",
      "- 2 criterios (dato/condición):",
      "  1) Condición: excepciones frecuentes → alto coste operativo y retrasos de pago.",
      "  2) Condición: reglas claras (match) → automatización defendible con auditoría.",
      "- Restricción dominante: Trazabilidad/auditoría (finanzas).",
      "- Trade-off: Acepto más esfuerzo por integración ERP para bajar errores de conciliación."
    ].join("\n")
  }
};
...
function exportLabToMarkdown(){
  ...
  const tmpl = LAB_TEMPLATES[t] || {};

  const decision = (tmpl.decision_note || [
    "Decisión final:",
    "- Proceso ganador: _(rellena)_",
    "- 2 criterios (dato/condición):",
    "  1) ",
    "  2) ",
    "- Restricción dominante: _(PII / auditoría / criticidad / latencia)_",
    "- Trade-off: _(qué sacrificas para qué)_"
  ]).join("\n");

  const aiLog = [
    "AI Log",
    "1) Herramienta/modelo: ChatGPT (GPT-5.2 Thinking)",
    "2) Objetivo del prompt: Generar inventario L1 + priorización + SIPOC con supuestos y sin PII.",
    "3) Prompt usado (3–8 líneas): Actúa como analista de procesos. Track: " + tName + ". Dame 20 procesos L1 (verbo+objeto) y luego una tabla Top 5 con Impacto/Esfuerzo/Riesgo (1–5) + nota observable. Por último, un SIPOC 1 página del ganador.",
    "4) Qué devolvió (resumen de 2–4 líneas): Lista de procesos, propuesta de Top 5 con scoring y un SIPOC inicial con suppliers/inputs/process/outputs/customers.",
    "5) Qué cambié yo (2–3 bullets): - Eliminé duplicados y convertí nombres a verbo+objeto.\n- Reescribí notas para que fueran condiciones observables y marqué supuestos.\n- Ajusté SIPOC a 4–6 pasos y marqué inputs con PII: posible/sí/no.",
    "6) Qué verifiqué (2 checks): ✅ Score usa la misma fórmula en todas las filas. ✅ SIPOC tiene Customers/Outputs claros y no incluye PII."
  ].join("\n");

  const md = [
    `# M2-S10 · Entregable · ${tName}`,
    "",
    "## Inventario (L1) — Verbo + Objeto",
    (inv || tmpl.inventory || "_(vacío)_"),
    "",
    "## Priorización (Top 5) — Impacto | Esfuerzo | Riesgo | Score | Nota",
    (pr || tmpl.prioritization || "_(vacío)_"),
    "",
    "## SIPOC (del proceso ganador)",
    (sip || tmpl.sipoc || "_(vacío)_"),
    "",
    "## Decisión (2 criterios + 1 restricción)",
    decision,
    "",
    aiLog
  ].join("\n");
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  // Lab: autocompletar con plantilla del track si está vacío
  if($("#inv") && !$("#inv").value.trim()) {
    const t = getTrack();
    const tmpl = LAB_TEMPLATES[t];
    if(tmpl){
      $("#inv").value = tmpl.inventory || "";
      $("#prio").value = tmpl.prioritization || "";
      $("#sipoc").value = tmpl.sipoc || "";
    }
  }
